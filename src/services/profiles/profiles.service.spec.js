...
describe('PROFILES SERVICE', () => {
  describe('GET PROFILE', () => {
    test('should return profile', async () => {
      jest.spyOn(UsersRepository, 'findOneBy').mockReturnValue({id: 1, following: [2]});
  ...
  describe('FOLLOW PROFILE', () => {
    ...
    async () => {
      jest.spyOn(ProfilesService, 'fetchDataFromDB').mockReturnValue([{following: [3]}, {id: 3}]);
    ...
    test('should follow a profile', async () => {
      jest.spyOn(UsersRepository, 'follow').mockImplementation(() => Promise.resolve());
      jest.spyOn(ProfilesService, 'fetchDataFromDB').mockReturnValue([{following: [3]}, {id: 2}]);
...
describe('UNFOLLOW PROFILE', () => {
  ...
  async () => {
    jest.spyOn(ProfilesService, 'fetchDataFromDB').mockReturnValue([{following: []}, {id: 2}]);
  ...
  test('should unfollow a profile', async () => {
    jest.spyOn(UsersRepository, 'unfollow').mockImplementation(() => Promise.resolve());
    jest.spyOn(ProfilesService, 'fetchDataFromDB').mockReturnValue([{following: [2]}, {id: 2}]);
