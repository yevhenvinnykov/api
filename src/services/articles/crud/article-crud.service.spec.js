...
describe('ARTICLES CRUD SERVICE', () => {
    ...
    test('should throw an error if the article is not created', async () => {
      ...
      try {
        ...
        expect(error.message).toBe(
          'Something went wrong when creating article'
        );
      }
    });
  });
  ...
  describe('UPDATE ARTICLE', () => {
    ...
    test('should throw an error if auth user is not the author', async () => {
      ...
      try {
        ...
        expect(error.message).toBe(
          'You are not authorized to update the article'
        );
      }
    });
  });
  ...
  describe('GET ARTICLE', () => {
    ...
    test('should get the article and add to it favorited&followed info', async () => {
    ...
    test(`should get the article and add favorited&followed: false
          because no authUserId provided`, async () => {
      const article = await ArticlesCRUDService.getArticle({
        slug: 'slug',
        authUserId: null,
      });
  ...
  describe('DELETE ARTICLE', () => {
    test('should delete an article', async () => {
      ...
      const fn = async () =>
        await ArticlesCRUDService.deleteArticle({
          slug: 'slug',
          authUserId: 1,
        });
