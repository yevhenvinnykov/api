...
describe('USER MIDDLEWARE', () => {
  ...
  beforeEach(() => {
    reqMock = {
      body: { user: { email: 'email@email.com', username: 'username' } },
    };
  ...
  describe('CHECK IF USER EXISTS', () => {
    ...
    test("when there's a user with such an email, res with corresponding error should be sent", async () => {
      jest
        .spyOn(UsersRepository, 'findOneByOr')
    ...
    test("when there's a user with such a username, res with corresponding error should be sent", async () => {
      jest
        .spyOn(UsersRepository, 'findOneByOr')
